...
- Owner can move `done` back to `planning` via improve if changes are needed
...
- **working**: Agent actively working (planning or executing)
...
- When `working` (after accept), orchestrator launches agent to EXECUTE
...
### Phase 1: Execute
1. Launch agents on working tasks (after accept)
2. Launch agents on planning tasks (after improve feedback)
