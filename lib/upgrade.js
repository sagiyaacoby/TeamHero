...
function backupPlatformFiles(ctx, version) {
  ...
  PLATFORM_FILES.forEach(function(pf) {
    var srcPath = ctx.path.join(ctx.ROOT, pf);
    var destPath = ctx.path.join(backupDir, pf);
...
function rollback(ctx, version) {
  ...
  var restored = 0;
  try {
    // Restore each platform file/directory from backup
    PLATFORM_FILES.forEach(function(pf) {
      var srcPath = ctx.path.join(backupDir, pf);
      var destPath = ctx.path.join(ctx.ROOT, pf);
